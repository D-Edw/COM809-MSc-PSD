...
//declare String array with initial values for days
        String[] days = {"Monday", "Tuesday", "Wednesday", "Thursday", "Friday"}; //declare a double array with initial values for times
        double[] time = {9.00, 10.00, 11.00, 12.00, 13.00, 14.00, 15.00, 16.00}; //declare a 2D array
...
//hardcode in the values and store in the array
        ...
        // print out the top line of times for the table
            ...
            for (int row = 0; row < DAYSMAX; row++) { System.out.print(days[row] + " \t");
                for (int column = 0; column < TIMESMAX; column++) {
            ...
            for (int row = 0; row < DAYSMAX; row++) {
                ...
                for (int column = 0; column < TIMESMAX; column++) {
                    if (searchTime == time[column]) { searchColumn = column;
                    }//if
                }//for

                    // Third, print out the entry in the array at the intersection of searchDay and searchTime
                    System.out.println(searchDay + " " + df.format(searchTime) + " - " + module[searchRow][searchColumn]);
                }//main
